...
class InvalidSourceModel(Exception):
    pass
	...
	try:
		...
		if area_sources != []:
			for source_tag in area_sources:
				...
				# points_list = [float(s) for s in area_text.split() if s.isdigit()]
				points_list = [float(s) for s in area_text.split()]
				points_list = list(chunks(points_list, 2))
				new_points_list = []
				for pt in points_list:
					new_points_list.append( ' '.join(str(coor) for coor in pt) )

				new_points_list.append(new_points_list[0])

				wkt = 'POLYGON((' + ', '.join(new_points_list) + '))'
		...
		if fault_sources != []:
			for source_tag in fault_sources:
				...
				# del points_list[0]
				# del points_list[-1]
				points_list = [float(s) for s in fault_text.split()]
				points_list = list(chunks(points_list, 2))
				new_points_list = []
				for pt in points_list:
					new_points_list.append( ' '.join(str(coor) for coor in pt) )

				wkt = 'LINESTRING(' + ', '.join(new_points_list) + ')'
		...
		exc_tb = sys.exc_info()
		line = exc_tb[2].tb_lineno
		raise InvalidSourceModel('Line: '+str(line)+ str(e))
